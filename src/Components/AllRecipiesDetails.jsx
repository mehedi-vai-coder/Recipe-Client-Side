...
const RecipeDetails = () => {
    ...
    const fetchRecipe = useCallback(async () => {
        setLoading(true);
        try {
            const res = await axios.get(`https://recipe-server-side-five.vercel.app/recipes/${id}`);
            setRecipe(res.data);
        } catch (err) {
            console.error(err);
    ...
    const handleLike = async () => {
        ...
        setLiked(true);
        try {
            const res = await axios.patch(`https://recipe-server-side-five.vercel.app/recipes/${id}/like`);
